...
void writeLcd(char* data, int min, int sec, int line)
{ 
  int a;
	int b;
	int c;
	char str[15] = {'A','c','t','u','a','l',':',' ',' ',' ',' ',' ',' ',' ',' '}; 
  ...
  str[7]=' ';
  if(min==6)    /* Wenn Preset eingestellt wird*/
  {
   str[8]='-';
   str[9]='-';
   str[10]=':';
  ...
  /*Umwandeln des Sekundenwerts in char*/
	a = ((sec/10) + 48); 
	b = ((sec%10) + 48);
	c = min + 48;
	str[8] = '0';
